...
async function register(params, origin) {
	...
	if (await db.Account.findOne({ email: params.email })) {
		// Send 'Already Registered' error by email to prevent account enumeration
...
async function sendAlreadyRegisteredEmail(email, origin) {
	let message;
	if (origin) {
		message = `<p>If you forgot your password, please visit the <a href="${origin}/account/forgot-password">forgot password</a> page.</p>`;
	} else {
		message = `<p>If you forgot your password you can reset it via the <code>/account/forgot-password</code> api route.</p>`;
	}
...
async function sendPasswordResetEmail(account, origin) {
	...
	if (origin) {
		...
		message = `<p>Please click the link below to reset your password. The link will be valid for 1 hour:</p>
